...
Server.prototype = {
    ...
    postModified: function(post) {
        if (post.isChronological) {
            post.blog.cache.remove(post.url);
            post.blog.cache.removeAll('/api/page');
            post.blog.app.cache.removeAll('/');
        } else {
            if (post.group == 'drafts') {
                post.blog.cache.remove('/api/group/drafts');
            } else {
                post.blog.cache.removeAll(post.url);
            }
        }
    },
