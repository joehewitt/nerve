...
Server.prototype = {
    configure: function(modulePaths, contentPaths, configName, cb) {
        ...
        async.map(modulePaths,
            _.bind(function(modulePath, cb2) {
                ...
                appjs.loadApp(modulePath, _.bind(function(err, app) {
                    assert(!err, "Unable to load app at " + modulePath);

                    var settings = readSettings(app.packageInfo, configName);
...
function readSettings(packageInfo, configName) {
    var settings = {};
    var nerveSettings = packageInfo.nerve;
    if (nerveSettings) {
    for (var name in nerveSettings) {
            if (name == 'configs') {
                var subsettings = nerveSettings[name][configName];
                if (subsettings) {
                    for (var name in subsettings) {
                        settings[name] = subsettings[name];
                    }
                }
            } else {
                settings[name] = nerveSettings[name];
            }
        }
    }
    return settings;
