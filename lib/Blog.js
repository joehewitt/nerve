...
subclass(Blog, events.EventEmitter, {
            ...
            _.each(statMap, ibind(function(mtime, filePath) {
                ...
                if (!postMap[filePath]) {
                    this._parsePostsFile(filePath, abind(function(err, posts) {
                        ...
                        // } {
                        //     D&&D('doing nothing because of firstload');
                        // }
                        next.apply(this);
                    }, cb, this));
                } else {
    ...
    _assignConf: function(conf) {
        ...
        if (conf.flickr) {
            ...
            this.addTransform(new FlickrEmbedder(conf.flickr.key, conf.flickr.secret));
        }

        // XXXjoe Not yet implemented
        // if (conf.facebook) {
        //     var FacebookEmbedder = require('./FacebookEmbedder').FacebookEmbedder;
        //     this.addTransform(new FacebookEmbedder(conf.facebook.key, conf.facebook.secret));
        // }
    },
