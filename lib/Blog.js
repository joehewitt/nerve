...
subclass(Blog, events.EventEmitter, {
    init: function(configName, options, monitor, cb) {
        this._findContent(this.contentPattern, abind(function(err) {
            appjs.loadApp(this.appPath, configName, options, abind(function(err, app) {
                ...
                this._useSettings(settings, monitor, abind(function(err) {
                    cb(0, this);
                }, cb, this));
            }, cb, this));
        }, cb, this));
    ...
    onPostModified: function(post) {
        if (post.isChronological) {
            // D&&D('+ before', require('util').inspect(this.cache, false, 100));
            this.cache.remove('/api/post/' + post.url);
            this.cache.removeAll('/api/page');
            this.cache.removeAll('/api/posts');
            // D&&D(' - after', require('util').inspect(this.cache, false, 100));
    ...
    renderEmbed: function(embed, post, cb) {
        ...
        this.cache.load(key, _.bind(function(err, entry) {
            if (err || !entry) {
                ...
                post.attach.apply(post, entry.attachments);
                cb(0, entry);
    ...
    _useSettings: function(settings, monitor, cb) {
